...
1. 安装团队指定的网络客户端，导入 VPN 订阅
2. 选择可访问 `chatgpt.com` 和 `github.com` 的节点
3. 注册 Gmail，并用它注册或登录 ChatGPT
4. 如走 iOS 内购，注册或切换可用的 Apple ID
5. 在 ChatGPT App 内完成订阅，并确认当前计划可用
6. 选择 Git 平台：研发人员可使用公司私有化 GitLab；非研发当前没有公司 GitLab 账号，先注册 GitHub 或 GitLab.com 个人账号
7. 非研发准备一个个人私有仓库；研发使用自己已有权限的公司 GitLab 仓库；然后 clone 到本地
8. 在 Codex App 中打开仓库根目录
9. 先发送只读检查提示词，不要立刻修改文件
10. 需要改内容时先用 Plan 模式，让 Codex 给计划
11. 计划确认后再让 Codex 修改
12. 修改完成后先让 Codex 总结 diff；个人工作内容由本人确认保存，涉及公司项目或共享仓库时再找研发同事确认
...
- 手机号验证优先使用本人长期可用手机号
- HeroSMS 这类第三方接码服务存在账号风控、隐私和合规风险，不作为团队标准流程
...
价格和额度会变化，以官方页面和客户端显示为准。截至 2026-05-10，个人常见档位可以这样理解：

| 档位 | 适合谁 | 说明 |
|------|--------|------|
| Plus，约 $20/月 | 非研发、轻量用户、初学者 | 适合文档、小改动和低频使用 |
| Pro 5x，约 $100/月起 | 高频研发、测试同事、工具维护同事 | 比 Plus 有更高 Codex 使用量 |
| Pro 20x，约 $200/月 | 重度研发、长任务、并行任务 | 更适合持续使用和重型任务 |

推广初期不建议全员直接购买高档位。轻量用户先用 Plus；高频研发、测试同事和工具维护同事再评估 Pro。当前先按个人自购路径执行，后续如果公司支持统一采购，再单独更新本页。
...
如果只是读文档，暂时不一定需要 SSH Key；如果要提交或推送，就需要配置。
...
| 角色 | 要完成的事 |
|------|------------|
| 全员 | 能登录 ChatGPT 和 Codex App，能打开个人私有仓库目录，知道密码、Token、验证码和私钥不能发给 AI |
| 非研发 | 能用个人私有仓库记录 PRD、日志、周报等工作内容，能让 Codex 先只读检查项目，能用 Plan 模式表达需求 |
| 研发 | 能指导同事创建私有仓库，确认 SSH、远程仓库和权限配置，说明分支或 worktree 的使用边界 |
